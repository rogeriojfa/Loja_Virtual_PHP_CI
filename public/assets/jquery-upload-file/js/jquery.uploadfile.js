	...
	$.fn.uploadFile = function (options) {
			...
			if (obj.existingFileNames.length) {
				for (var x = 0; x < obj.existingFileNames.length; x++) {
					if (
						...
						(s.duplicateStrict &&
							obj.existingFileNames[x].toLowerCase() == filename.toLowerCase())
					) {
						duplicate = true;
					}
				}
			...
			fileInput.change(function () {
				...
				if (s.serialize && feature.fileapi && feature.formdata) {
					...
					for (var i = 0; i < fileArray.length; i++) {
						if (s.showFileCounter)
							fileList +=
								obj.fileCounter + s.fileCounterStyle + fileArray[i] + "<br>";
		...
		function defaultProgressBar(obj, s) {
			...
			this.progressDiv = $("<div class='ajax-file-upload-progress'>")
				.appendTo(this.statusbar)
				.hide();
			this.progressbar = $("<div class='ajax-file-upload-bar'></div>").appendTo(
				this.progressDiv
			);
		...
		function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
			...
			var options = {
				...
				beforeSubmit: function (formData, $form, options) {
					...
					pd.statusbar.append(
						"<div class='" + s.errorClass + "'>" + s.uploadErrorStr + "</div>"
					);
					pd.cancel.show();
					form.remove();
				...
				success: function (data, message, xhr) {
					...
					if (
						...
						if (s.showStatusAfterError) {
							pd.progressDiv.hide();
							pd.statusbar.append(
								"<span class='" + s.errorClass + "'>ERROR: " + msg + "</span>"
							);
						} else {
							pd.statusbar.hide();
							pd.statusbar.remove();
