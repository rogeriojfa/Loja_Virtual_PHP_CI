...
class Usuarios extends CI_Controller {
    ...
    public function core($usuario_id = NULL) {
        ...
        if (!$usuario_id) {
            ...
            $this->form_validation->set_rules('email', 'Email', 'trim|required|min_length[4]|max_length[250]|valid_email|callback_valida_email');
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->ion_auth->register($username, $password, $email, $additional_data, $group)) {
                    ...
                    $this->session->set_flashdata('sucesso', 'Usuário incluído com sucesso');
                } else {
            ...
            if (!$usuario = $this->ion_auth->user($usuario_id)->row()) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    //Não atualiza senha se não vier senha
                    if (!$password) {
                        unset($data['password']);
                    }
    ...
    public function valida_email($email) {
        ...
        if (!$usuario_id) {
            ...
            if ($this->core_model->get_by_id('users', array('email' => $email))) {
                $this->form_validation->set_message('valida_email', 'O e-mail informado já existe.');
                return false;
            } else {
                return true;
            }
    ...
    public function valida_usuario($username) {
        ...
        if (!$usuario_id) {
            ...
            if ($this->core_model->get_by_id('users', array('username' => $username))) {
                $this->form_validation->set_message('valida_usuario', 'O usuário informado já existe.');
                return false;
            } else {
                return true;
            }
            ...
            if ($this->core_model->get_by_id('users', array('username' => $username, 'id != ' => $usuario_id))) {
                $this->form_validation->set_message('valida_email', 'O usuário informado já existe.');
                return false;
            } else {
                return true;
            }
